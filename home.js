...
document.addEventListener("DOMContentLoaded", () => {
    // 1. Listen for the signal from shared.js that the database has finished loading!
    ...
    // 2. Initialize the calendar grid (will draw empty first, then fill when DB loads)
    if (document.getElementById('calendar-grid')) {
...
window.updateDynamicCalendar = function() {
    ...
    for (const [catKey, events] of Object.entries(window.EVENTS_DATA || {})) {
        events.forEach(ev => {
            // Attempt to parse the date string from the database
            const d = new Date(ev.date);
            
            // Only add to calendar if the date is valid (ignores "TBD" or weird strings)
            if (!isNaN(d)) {
                ...
                if (ev.date && ev.date.includes(',')) {
                    ...
                    extractedTime = ev.date.match(/\d{1,2}:\d{2}\s?(AM|PM|am|pm)?/)?.[0] || "TBA";
                }
